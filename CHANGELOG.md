...
## 0.1.9-beta: точный статус Adaptive Live в Console

- Исправлен ложный статус «Есть отклонения»: Console API больше не ищет DNS-фильтр в обрезанном хвосте вывода `ps w`.
- Счётчик `tcpdump` использует устойчивые маркеры того же Adaptive Live capture, которые проверяет штатный сервис `S91adaptive-live`.
- Добавлен регрессионный тест для ограниченного вывода BusyBox `ps`; бизнес-логика AdaptiveAuto и маршрутизация не изменялись.
