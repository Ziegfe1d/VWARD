...
## 0.2.0-rc.1.fix.13: исправление RC1

Обновления
- Новый раздел «Обновления»: VWARD, AdGuard Home, прошивка Keenetic и пакеты Entware,
  у каждого своя страница с версией, доступной версией, переключателем «Обновлять
  автоматически» и каналом, где он есть. Прошивку Keenetic VWARD не ставит, только
  переключает её автообновление и канал.
- AdGuard Home и пакеты Entware обновляются через opkg: перед установкой копия файлов
  и базы opkg, после - проверка DNS и консоли; при сбое возвращается прежняя версия.
  Системные пакеты (busybox, libc, opkg, dropbear) - только вручную.
- Движок обновлений 2.0: скачивает и заменяет только изменившиеся файлы и обновляет
  сам себя. Старый движок перейдёт на него сам в течение часа после этой установки.

Нагрузка на роутер
- Реклама: проверка журнала запросов берёт у AdGuard Home только новые записи
  вместо последних 20 000.
- Фоновые задания читают настройки роутера и Wi-Fi клиентов через RCI: в журнале
  Keenetic больше нет тысяч строк о сессиях VWARD.

Безопасность и надёжность (глубокий аудит)
- API отвечает только на адрес роутера и его имена (защита от DNS rebinding).
- Скачанная резервная копия - без ключей туннелей и паролей.
- Ключи туннеля передаются роутеру не в командной строке, а в теле запроса RCI.
- Зависшие после выключения питания блокировки снимаются сами.
- Очистка старых журналов и копий работает на BusyBox роутера.

## 0.2.0-rc.1.fix.12: исправление RC1

Консоль
- «Что нового»: нажатие на установленную версию или на доступное обновление
  открывает список изменений этой версии.
- Окно установки обновления как в Keenetic: кольцо с процентами и этап под ним,
  в конце — установленная версия.
- Состояние блока показывается в его заголовке (Обновления, Интернет, туннель,
  компоненты) вместо повторяющейся строки «Состояние».
- Выпадающие списки прямо под полем вместо полноэкранного выбора телефона.
- Строка «Данные на …» на обзоре больше не выталкивает кнопку «Настроить»;
  при отсутствии ответа роутера об этом написано.

Скорость
- Реклама: список источников и счётчики решений собираются за один проход
  (раньше — около 40 отдельных запусков jq).
- Списки: хранятся только счётчики адресов, а не ответ роутера на ~200 КБ;
  устаревшие счётчики показываются сразу и обновляются в фоне.
- Маршруты: счётчики каталога доменов пересчитываются только при его изменении.

## 0.2.0-rc.1.fix.11: исправление RC1

Роутер и DNS
- Smart DNS в AdGuard Home: домены `[/домен/]https://...` из AdGuard Home теперь
  видны защите Smart DNS (AdaptiveAuto не уводит их в VPN), предупреждению о
  конфликте в списках и диагностике. Список, переведённый на VPN, временно
  убирает свои строки Smart DNS и из AdGuard Home (через его API, с проверкой и
  откатом) и возвращает их при возврате в обход.
- Сторож движка маршрутов больше не принимает собственные подпроцессы движка за
  второй движок: ложные перезапуски во время проверки доменов прекратились.
  Причина каждого перезапуска пишется в журнал событий.
- В Keenetic уходят только корректные имена доменов (при обычной работе и при
  восстановлении AdaptiveAuto после перезагрузки) — без «argument parse error».
- Цепочка IP-категорий не обрывается, когда аудиту нечего проверять (правка,
  сделанная на роутере, перенесена в пакет).
- Консоль берёт конфигурацию Keenetic не чаще раза в 10 секунд, доменные группы
  с адресами — раз в минуту; любое изменение сбрасывает сохранённое.
- Ежедневная копия настроек проверяется раз в сутки, а не каждый час.

Консоль
- Диагностика: «Цепочка DNS» (Keenetic → AdGuard Home → серверы, петля на
  роутер), «Файлы VWARD» (ручные правки установленных файлов), Smart DNS из
  AdGuard Home. Списки показывают, сколько адресов узнал Keenetic.
- Доступ только с зарегистрированных в Keenetic устройств (по умолчанию
  выключено; включить можно только с зарегистрированного устройства, роутер
  допускается всегда, выключение по SSH: `vward-console-config.sh console-devices 0`).
- Реклама: раздел начинается с проверки VWARD (состояние, последняя проверка,
  итоги, куда уходят правила, последние решения); «Проверить домен» показывает
  ответ на странице; настройки AdGuard Home — на отдельной странице.
- Резервные копии настроек VWARD (ежедневно и по кнопке, 7 последних,
  восстановление с копией текущего состояния).
- Файлы VWARD: просмотр и скачивание, ключи и пароли закрыты.
- Wi-Fi: имена клиентов из Keenetic, переименование и запрет интернета.
- Обновления: окно с этапами установки, «Установить» ставит сразу, спокойные
  уведомления; каналы Stable/Beta/Dev.
- Статусы точкой и текстом, понятные формулировки вместо «fail-open», подписи
  нижней панели целиком на телефонах, мгновенное переключение режимов, данные
  из кэша браузера при открытии.

## 0.2.0-rc.1.fix.10: исправление RC1

- Реклама: настройки блокировки самого AdGuard Home прямо со страницы VWARD —
  защита, фильтрация и частота обновления списков, безопасная навигация,
  родительский контроль, безопасный поиск; страница «Фильтры AdGuard Home»
  (включить, выключить, добавить, удалить, обновить списки) и «Блокировка
  сервисов». Каждое изменение перечитывается из AdGuard Home. Выключение защиты
  и удаление списка требуют подтверждения.
- Логин AdGuard Home передаётся curl через stdin во всех запросах, а не в
  аргументах процесса.

## 0.2.0-rc.1.fix.9: исправление RC1

- IP-категории на реальном KN-1913 включились (4 категории, 109 подсетей), но все
  109 маршрутов роутер отклонил: Keenetic принимает в `ip route` имя интерфейса
  туннеля, а не системное имя устройства. Маршруты теперь добавляются и
  убираются по имени интерфейса; принадлежность маршрутов VWARD записывается
  так же (переключение туннеля в Console тоже).
- Источник Loyalsoldier: список файлов с GitHub разбирался многострочной заменой
  sed, которую BusyBox не понимает («sed: unmatched '/'»). Заменено на awk.

## 0.2.0-rc.1.fix.8: исправление RC1

- IP-категории не включались («0 активны», «Маршрутов VWARD: 0»), хотя каталоги
  itdog и v2fly скачались. Keenetic пишет в маршрутах доменных списков имя
  интерфейса туннеля, а синхронизация, сверка и аудит IP-категорий искали только
  системное имя устройства, поэтому не видели ни одного списка через VPN. Теперь
  учитываются оба имени; тест воспроизводит конфигурацию роутера.
- Всё из fix.7 (fix.7 опубликован, но не устанавливался).

## 0.2.0-rc.1.fix.7: исправление RC1

- Туннели из VWARD. «Заменить конфигурацию» на странице туннеля: новый .conf
  (WireGuard или AmneziaWG 2.0) сначала проверяется на временном туннеле, и только
  когда сервер ответил, записывается в тот же туннель — имя, маршруты и списки
  остаются. Любой отказ роутера или сервера откатывает изменения; конфигурация
  роутера сохраняется только после ответа сервера. Keenetic скрывает приватный
  ключ, поэтому VWARD хранит три последних применённых .conf (только root).
  «Создать туннель» на странице VPN, подсети и доменные списки через любой туннель,
  безопасное удаление туннеля: сначала его списки и подсети переходят в туннель
  VWARD, другой туннель или к провайдеру. Туннель VWARD удалить нельзя.
- Защита Smart DNS: AdaptiveAuto не отправляет в VPN домены Smart DNS (их общий
  адрес прокси через туннель не работает); страница списков и уведомления
  предупреждают о таких доменах в списках через VPN; диагностика проверяет, что
  адреса Smart DNS идут через провайдера. Работает с любым DNS-over-HTTPS, есть
  выключатель.
- AdGuard Home: порт берётся из AdGuardHome.yaml (на роутере 3001, VWARD искал 3000).
  На странице «Реклама» — подключение логином и паролем AdGuard Home: VWARD
  проверяет их у AdGuard Home и хранит в файле только для root; пароль не
  попадает в аргументы процессов и журналы.
