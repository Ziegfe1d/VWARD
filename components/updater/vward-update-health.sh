...
case "$profile" in
    ...
    tunnel-guard)
        ...
        ;;
    wan-guard)
        required="$VU_ROOT_PREFIX/opt/bin/wan-health-watch.sh $VU_ROOT_PREFIX/opt/bin/wan-capability.sh $VU_ROOT_PREFIX/opt/bin/wan-recovery-plan.sh $VU_ROOT_PREFIX/opt/bin/wan-recovery-actuator.sh $VU_ROOT_PREFIX/opt/bin/wan-recovery-controller.sh $VU_ROOT_PREFIX/opt/bin/wan-guardian.sh"
        ;;
    policy-sync)
