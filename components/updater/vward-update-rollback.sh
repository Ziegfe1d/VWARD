...
[ -n "$expected_index_sha" ] && [ "$expected_index_sha" = "$actual_index_sha" ] || rollback_fail "Backup index hash mismatch"
...
if [ "$(sed -n '1p' "$backup/committed.existed" 2>/dev/null || :)" = 1 ]; then
    ...
    [ -n "$expected_committed_sha" ] && [ "$expected_committed_sha" = "$actual_committed_sha" ] || rollback_fail "Committed metadata backup hash mismatch"
fi
...
while IFS="$(printf '\t')" read -r target existed mode original_sha backup_sha; do
    ...
    if [ "$existed" = 1 ]; then
        ...
        [ "$actual_backup_sha" = "$backup_sha" ] && [ "$original_sha" = "$backup_sha" ] || rollback_fail "Backup payload hash mismatch for $target"
    fi
...
while IFS="$(printf '\t')" read -r target existed mode original_sha backup_sha; do
    ...
    if [ "$existed" = 1 ]; then
        ...
        cp "$source" "$tmp" && chmod "$mode" "$tmp" && sync && mv -f "$tmp" "$destination" || rollback_fail "Cannot restore $target"
