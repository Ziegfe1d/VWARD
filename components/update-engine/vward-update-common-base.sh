...
vu_runtime_resume() {
    ...
    if [ -z "$VU_ROOT_PREFIX" ]; then
        ...
        if [ "$VU_RESTART_SUPERVISOR" = 1 ]; then
            /opt/etc/init.d/S92vward-runtime start >/dev/null 2>&1 || resume_rc=1
        fi
...
vu_barrier_enter() {
    ...
    VU_BARRIER_OWNED=1
    [ -z "$VU_ROOT_PREFIX" ] || [ "${VWARD_TEST_CONFLICT_AFTER_BARRIER:-0}" != 1 ] || : > "$VU_ROOT_PREFIX/tmp/vward-wan-guard.lock"
    vu_activity_clear || return 1
    return 0
}
