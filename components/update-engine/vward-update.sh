...
archive_declared_size() {
    package=$1
    listing=$2
    tar -tvzf "$package" > "$listing" 2>/dev/null || return 1
    awk '
      BEGIN { sum=0 }
      {
        type=substr($1,1,1)
...
download_and_unpack() {
    ...
    expected_unpacked=$(vu_manifest_unpacked_size "$manifest") || vu_die "$VU_VERIFY_ERROR" "Missing or invalid unpacked size"
    [ "$expected_size" -le "$max_package_size" ] || vu_die "$VU_SAFETY_ERROR" "Package size exceeds configured maximum"
    [ "$expected_unpacked" -le "$max_unpacked_size" ] || vu_die "$VU_SAFETY_ERROR" "Unpacked size exceeds configured maximum"

    if [ -r "$VU_PENDING_DIR/package.tar.gz" ] && [ "$(sha256sum "$VU_PENDING_DIR/package.tar.gz" | awk '{print $1}')" = "$expected" ]; then
        cached_size=$(wc -c < "$VU_PENDING_DIR/package.tar.gz" | tr -d ' ')
        [ "$cached_size" = "$expected_size" ] || rm -f "$VU_PENDING_DIR/package.tar.gz"
    fi
    if [ -r "$VU_PENDING_DIR/package.tar.gz" ]; then
        ...
        test_size=$(wc -c < "$VWARD_TEST_PACKAGE" | tr -d ' ')
        [ "$test_size" -le "$expected_size" ] || vu_die "$VU_VERIFY_ERROR" "Test package exceeds signed size"
        cp "$VWARD_TEST_PACKAGE" "$package" || vu_die "$VU_VERIFY_ERROR" "Cannot copy test package"
    else
        url=$(jq -r '.signed.package.url' "$manifest")
        vu_fetch_bounded "$url" "$package" "$expected_size" || vu_die "$VU_NETWORK_ERROR" "Package download failed or exceeded signed size"
    fi
    ...
    listing=$transaction_staging/tar.list
    declared=$(archive_declared_size "$package" "$listing") || vu_die "$VU_VERIFY_ERROR" "Package links, special entries or malformed tar listing are forbidden"
    [ "$declared" -eq "$expected_unpacked" ] || vu_die "$VU_VERIFY_ERROR" "Signed unpacked size does not match archive"
    [ "$declared" -le "$max_unpacked_size" ] || vu_die "$VU_SAFETY_ERROR" "Archive unpacked size exceeds configured maximum"
...
quarantine_after_rollback() {
    manifest=$1
    reason=$2
    vu_quarantine_store "$manifest" "$reason" || vu_die "$VU_INSTALL_ERROR" "Rollback succeeded but quarantine could not be persisted"
    vu_log WARN "Update $(jq -r '.signed.update_id' "$manifest") quarantined after $reason failure"
}

apply_update() {
    manifest=$1
    package_dir=$2
    vu_barrier_request || vu_die "$VU_SAFETY_ERROR" "Could not publish the update request"
    ...
    if ! install_package "$package_dir"; then
        ...
        if [ "$rollback_policy" = automatic ]; then
            if VWARD_INTERNAL_ROLLBACK=1 VWARD_INTERNAL_ROLLBACK_TOKEN="$VU_LOCK_TOKEN" "$SELF_DIR/vward-update-rollback.sh" "$backup"; then
                quarantine_after_rollback "$manifest" install
                vu_die "$VU_INSTALL_ERROR" "Install failed; rollback completed and update quarantined"
            fi
    ...
    if ! run_health_bounded "$profile"; then
        ...
        if [ "$rollback_policy" = automatic ]; then
            if VWARD_INTERNAL_ROLLBACK=1 VWARD_INTERNAL_ROLLBACK_TOKEN="$VU_LOCK_TOKEN" "$SELF_DIR/vward-update-rollback.sh" "$backup"; then
                quarantine_after_rollback "$manifest" health
                vu_die "$VU_HEALTH_ERROR" "Health check failed; rollback completed and update quarantined"
            fi
...
case "$command" in
    ...
    --recover)
        ...
        vu_barrier_request || vu_die "$VU_SAFETY_ERROR" "Cannot publish recovery request"
...
case "$fetch_rc" in
    ...
    "$VU_QUARANTINED") [ "$command" = --dry-run ] || vu_transition FAILED; exit "$VU_QUARANTINED" ;;
    "$VU_NETWORK_ERROR") vu_die "$VU_NETWORK_ERROR" "Manifest download failed" ;;
    "$VU_VERIFY_ERROR") vu_die "$VU_VERIFY_ERROR" "Manifest structure/signature verification failed" ;;
    "$VU_COMPAT_ERROR") vu_die "$VU_COMPAT_ERROR" "Manifest violates version, channel, replay or compatibility policy" ;;
    "$VU_SAFETY_ERROR") vu_die "$VU_SAFETY_ERROR" "Manifest/package staging safety preflight failed" ;;
    *) vu_die "$fetch_rc" "Manifest processing failed" ;;
esac
