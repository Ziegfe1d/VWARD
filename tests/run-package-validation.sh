...
busybox sh -n "$ROOT/web/fragments/api-ads-privacy-guard.fragment.cgi"
find "$ROOT" -type f -name '*.json' -print | while IFS= read -r f; do jq -e . "$f" >/dev/null; done
if command -v node >/dev/null 2>&1; then node --check "$ROOT/web/fragments/ads-privacy-guard-console.fragment.js" >/dev/null; fi
...
if grep -RInEi 'BEGIN (RSA|OPENSSH|EC|PRIVATE) KEY|private[_-]?key[=:]|password[=:][^[:space:]]|token[=:][A-Za-z0-9_-]{12,}|api[_-]?key[=:][A-Za-z0-9_-]{12,}|aeternia' \
  ...
  "$ROOT/config/route-engine" "$ROOT/web/fragments" \
