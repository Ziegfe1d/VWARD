...
Открыто до публикации RC1:

1. **Переход beta → 0.2 (P0).** На beta работают старые скрипты
   (`agh-adaptive-live.sh`, `vpn-subnet-sync.sh`, `/opt/share/keenetic-apps/www`...).
   Пакет 0.2 ставит канонические файлы рядом, но не переключает cron
   (`config/cron/root.crontab`), init-скрипты, lighttpd, `device.conf` и state. После
   перезагрузки init-скрипты `S9x` из пакета могут запуститься параллельно со старой
   установкой.
   Нужен cutover-установщик: backup, остановка старых заданий, установка cron и init
   0.2, discovery Device Profile, проверка и обратный переход.
2. Проверки на роутере из [`CONSOLE_REWORK.md`](CONSOLE_REWORK.md):
   - синтаксис `dns-proxy route object-group`;
   - формат ответа `/auth`;
   - имена полей WireGuard в RCI.
...
4. Роутеры, у которых `manifest_url` указывает на feed ветки `dev`, получат обновление.
   ...
   переходят: их переводит cutover (пункт 1 выше). На установленной 0.2 канал выбирается
   в Console («Обновления → Канал», подтверждение `UPDATE_FEED_DEV`).
