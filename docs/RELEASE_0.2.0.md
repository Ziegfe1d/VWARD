...
Срочное исправление внутри этапа получает суффикс `.fix.N`: `0.2.0-rc.1.fix.1`,
`0.2.0-rc.1.fix.2`. Updater сравнивает версии по SemVer, и такая версия новее
`0.2.0-rc.1`, но старше `0.2.0-rc.2`. Формы `0.2.0-rc1-fix1` и `0.2.0-rc.1-fix.1` не
годятся: по SemVer они старше `0.2.0-rc.2`, и роутер с ними никогда не обновился бы до RC2.
Опубликованный пакет не перезаписывается, поэтому исправление в компонентах всегда
выходит под новой версией.
