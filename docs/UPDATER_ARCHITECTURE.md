...
## Движок 2.0: пофайловые обновления и самообновление

Для ленты схемы 2 шаги 7-9 заменяются так:

7. Сравнить подписанный список файлов с роутером (один `sha256sum` и `ls -ln`).
8. Скачать только отличающиеся файлы одним запуском curl, каждый - с проверкой
   размера и SHA-256; скачанное хранится в `pending/files` до установки.
9. Собрать из них пакет, по которому работают те же резервная копия, замена,
   health и rollback; перезапускаются только компоненты с изменёнными файлами.

Перед шагом 7, если в ленте движок новее, он проходит самопроверку, ставится в
неактивный слот A/B и перехватывает обновление. Подробности - в
`components/update-engine/README.md`.

## Следующие gates

- signed file removal только для реальной component consolidation;
- проверка движка 2.0 на роутере после выпуска.
