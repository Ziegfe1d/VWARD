...
> **Статус 0.2.0-rc.1.fix.11:** безопасного универсального установщика всей платформы пока
...
Настройки из Console записывает `vward-console-config.sh` (строгие значения, backup,
атомарная запись, проверка результата, журнал аудита); полный список - в `docs/CONSOLE.md`.

Вход по учётной записи Keenetic по умолчанию выключен. Включается в «Настройки → Доступ к
Console» вводом логина и пароля от веб-интерфейса роутера: пароль проверяет сам роутер
